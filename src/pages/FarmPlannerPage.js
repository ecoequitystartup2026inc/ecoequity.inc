...
const conditionIcon = (cond, size = 30) => {
  if (cond === "Sunny") return <Sun size={size} color="var(--eco-c7)" />;
  if (cond === "Partly Cloudy") return <CloudSun size={size} color="var(--eco-c7)" />;
  if (cond === "Cloudy") return <Cloud size={size} color="#64748b" />;
  if (cond === "Light Rain") return <CloudRain size={size} color="var(--eco-c9)" />;
  if (cond === "Thunderstorms") return <CloudRain size={size} color="#4338ca" />;
  return <CloudSun size={size} color="var(--eco-c7)" />;
};
  ...
  return (
    <div style={{ ...styles.wrap, ...(isMobile ? styles.wrapMobile : {}) }}>
      <style>{`
        ...
        .fp-dot { animation: fpDotPulse 2.4s ease-in-out infinite; }
        .fp-hero img { transition: transform .8s cubic-bezier(.22,1,.36,1); }
        .fp-hero:hover img { transform: scale(1.04); }
        @media (prefers-reduced-motion: reduce) {
          .fp-dot { animation: none; }
          .fp-hero img, .fp-hero:hover img { transition: none; transform: none; }
        }
      `}</style>
      <RevealStyles />
      ...
      <div style={styles.headerRow}>
        ...
        <div className="inner-blur-glass glass-hover-zoom-sm" style={styles.badge}>
          <span className="fp-dot" style={styles.badgeDot} />
      ...
      <Reveal variant="scale" delay={200} className="fp-hero" style={{ ...styles.hero, ...(isMobile ? styles.heroMobile : {}) }}>
        ...
        <span style={styles.heroCaption}>
          Region-by-region outlook for the season ahead
        </span>
      </Reveal>

      {/* Region selector */}
      <div style={styles.regionRow}>
        <MapPin size={16} color="var(--eco-c11)" />
        {REGIONS.map((r) => (
          <button
            key={r}
            type="button"
        ...
        <div style={{ ...styles.forecastRow, ...(isMobile ? styles.forecastRowMobile : {}) }}>
          {forecast.map((f, i) => (
            <div key={i} style={{ ...styles.forecastCard, ...(i === 0 ? styles.forecastCardToday : {}) }}>
              ...
              <span style={styles.forecastRain}><Droplet size={11} color="var(--eco-c9)" /> {f.rain}%</span>
            </div>
          ))}
        </div>
        <div style={styles.weatherMeta}>
          <span style={styles.metaItem}><Droplet size={13} color="var(--eco-c9)" /> Humidity {profile.humidity}%</span>
          ...
          {plantNow.length === 0 ? (
            ...
            <div style={styles.cropList}>
              {plantNow.map((h) => (
                ...
                >
                  <span style={styles.cropIcon}>
                    <CropThumb name={h.name} fallback={h.icon || <Wheat size="1em" color="var(--eco-c9)" />} rounded="8px" />
                  </span>
      ...
      <div className="inner-blur-glass" style={styles.card}>
        ...
        <div style={styles.legendRow}>
          <span style={styles.legendItem}><span style={{ ...styles.legendSwatch, background: "rgba(var(--eco-c9-rgb), 0.25)" }} /> Harvest season</span>
          <span style={styles.legendItem}><span style={{ ...styles.legendSwatch, background: "var(--eco-c9)" }} /> Peak month</span>
          <span style={styles.legendItem}><span style={{ ...styles.legendSwatch, background: "var(--eco-c7)" }} /> Plant now</span>
        </div>
              ...
              {regionHarvests.map((h) => (
                <tr key={h.id}>
                  <td style={styles.calCropCell}>
                    <span style={{ marginRight: 6 }}>{h.icon || <Wheat size="1em" color="var(--eco-c9)" />}</span>
                    {h.name}
                  </td>
                  {MONTHS.map((m) => {
                    ...
                    let bg = "transparent";
                    if (isPeak) bg = "var(--eco-c9)";
                    else if (inSeason) bg = "rgba(var(--eco-c9-rgb), 0.25)";
                    else if (isPlant) bg = "var(--eco-c7)";
...
const styles = {
  ...
  heroImg: { display: "block", width: "100%", aspectRatio: "21 / 7", objectFit: "cover" },
  heroScrim: {
    position: "absolute",
    inset: "auto 0 0 0",
    height: "62%",
  ...
  regionChipHov: { background: "rgba(var(--eco-c9-rgb), 0.12)", borderColor: "rgba(var(--eco-c9-rgb), 0.3)", color: "var(--eco-c13)", transform: "translateY(-1px)" },
  regionChipActive: { background: "linear-gradient(135deg, var(--eco-c9), var(--eco-c11))", color: "#fff", borderColor: "transparent", boxShadow: "0 6px 16px rgba(var(--eco-c9-rgb), 0.3)" },
  ...
  cardIcon: { color: "var(--eco-c13)", width: "20px", height: "20px" },
  ...
  forecastCardToday: { background: "linear-gradient(160deg, rgba(var(--eco-c5-rgb), 0.4), rgba(255,255,255,0.7))", border: "1px solid rgba(var(--eco-c9-rgb), 0.3)" },
  ...
  forecastRain: { display: "inline-flex", alignItems: "center", gap: "3px", fontSize: "11px", fontWeight: 600, color: "var(--eco-c13)", marginTop: "4px" },
  ...
  advisoryWet: { background: "rgba(var(--eco-c9-rgb), 0.1)", border: "1px solid rgba(var(--eco-c9-rgb), 0.25)" },
  advisoryDry: { background: "rgba(var(--eco-c7-rgb), 0.12)", border: "1px solid rgba(var(--eco-c7-rgb), 0.3)" },
  advisoryMild: { background: "rgba(var(--eco-c9-rgb), 0.1)", border: "1px solid rgba(var(--eco-c9-rgb), 0.25)" },
  ...
  cropItemHov: { transform: "translateY(-2px)", borderColor: "rgba(var(--eco-c9-rgb), 0.35)", boxShadow: "0 8px 20px rgba(var(--eco-c9-rgb), 0.12)" },
  /* Fixed box so a crop photo and an icon fallback occupy the same footprint. */
  cropIcon: { fontSize: "20px", display: "flex", alignItems: "center", justifyContent: "center", width: "34px", height: "34px", flexShrink: 0, borderRadius: "8px", overflow: "hidden" },
  ...
  cropTag: { fontSize: "11px", fontWeight: 700, color: "var(--eco-c13)", background: "var(--eco-c3)", padding: "4px 9px", borderRadius: "999px", whiteSpace: "nowrap" },
  cropTagPeak: { color: "#fff", background: "linear-gradient(135deg, var(--eco-c9), var(--eco-c11))" },
  ...
  calThNow: { color: "var(--eco-c13)", textDecoration: "underline" },
  ...
  cta: { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: "8px", padding: "13px 26px", borderRadius: "999px", border: "1px solid rgba(255,255,255,0.35)", background: "linear-gradient(135deg, var(--eco-c9), var(--eco-c11))", color: "#fff", fontSize: "14px", fontWeight: 700, fontFamily: "inherit", cursor: "pointer", boxShadow: "0 12px 28px rgba(var(--eco-c9-rgb), 0.3), inset 0 1px 0 rgba(255,255,255,0.4)", transition: "transform 0.2s ease, box-shadow 0.2s ease", marginTop: "4px" },
  ctaHov: { transform: "translateY(-2px) scale(1.02)", boxShadow: "0 16px 34px rgba(var(--eco-c9-rgb), 0.4), inset 0 1px 0 rgba(255,255,255,0.4)" },
};
